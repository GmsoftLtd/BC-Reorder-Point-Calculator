...
codeunit 50399 "Reorder Point Demo Data"
{
    ...
    // so the Reorder Point calculator has demand history and lead time to work with.
    ...
    var
        ...
        ResultTxt: Label 'Demo data created for item %1:\\Past Purchase Orders (post these FIRST to build inventory and lead time): %2\Past Sales Orders (post AFTER the POs to create demand history): %3\Future Sales Orders (leave unposted; for visual / MRP reference): %4\\Tip: open Sales Orders / Purchase Orders and use Post Batch.';
    begin
        if not EnvInfo.IsSandbox() then
            Error('Reorder Point Demo Data is a sandbox-only utility. The current environment is a production environment.');

        if not Item.Get(ItemNo) then
            Error('Item %1 does not exist.', ItemNo);

        EarliestDate := CalcDate('<-365D>', Today);
        ...
        for i := 1 to 6 do begin
            OrderDate := CalcDate(StrSubstNo('<-%1D>', 30 + Random(330)), Today);
            ReceiptDate := CalcDate(StrSubstNo('<+%1D>', 5 + Random(12)), OrderDate);
        ...
        for i := 1 to 30 do begin
            PostingDate := CalcDate(StrSubstNo('<-%1D>', 1 + Random(364)), Today);
            Qty := 1 + Random(14);
    ...
    begin
        ...
        if InvSetup."Location Mandatory" then begin
            ...
            if not Location.FindFirst() then
                Error('Inventory Setup requires a Location, but no non-in-transit Location exists. Create a Location before generating demo data.');
            exit(Location.Code);
        end;
        exit('');
    end;
    ...
    begin
        ...
        PH.Validate("Vendor Invoice No.", CopyStr('INV-' + PH."No.", 1, MaxStrLen(PH."Vendor Invoice No.")));
        if LocationCode <> '' then
